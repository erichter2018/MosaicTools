...
"
    ...
    private TabPage CreateAdvancedTab()
    {
        ...
        // DEV ONLY: Keystroke Discovery
        var discoverBtn = new Button
        {
            Text = "Discover Mosaic Shortcuts",
            Location = new Point(20, y),
            Size = new Size(180, 28),
            BackColor = Color.FromArgb(80, 50, 80),
            ForeColor = Color.White,
            FlatStyle = FlatStyle.Flat,
            Cursor = Cursors.Hand
        };
        discoverBtn.FlatAppearance.BorderColor = Color.Gray;
        discoverBtn.Click += (s, e) => RunKeystrokeDiscovery(discoverBtn);
        tab.Controls.Add(discoverBtn);

        var discoverHint = new Label
        {
            Text = "DEV ONLY: Tests key combos to find undocumented shortcuts",
            Location = new Point(210, y + 5),
            AutoSize = true,
            ForeColor = Color.FromArgb(120, 80, 120),
            Font = new Font("Segoe UI", 8, FontStyle.Italic)
        };
        tab.Controls.Add(discoverHint);

        return tab;
    }

    private KeystrokeDiscovery? _discovery;

    private void RunKeystrokeDiscovery(Button btn)
    {
        ...
        if (_discovery != null)
        {
            ...
            _discovery.Dispose();
            _discovery = null;
            btn.Text = "Discover Mosaic Shortcuts";
            btn.BackColor = Color.FromArgb(80, 50, 80);
            return;
        }

        var result = MessageBox.Show(
            "This will test ~300 key combinations in Mosaic to discover shortcuts.\n\n" +
            "REQUIREMENTS:\n" +
            "• Mosaic must be open with a report (preferably a test/blank one)\n" +
            "• Don't touch the mouse or keyboard during the scan\n" +
            "• Takes about 2-3 minutes\n\n" +
            "Results will be saved to 'discovered_shortcuts.txt' in the app folder.\n\n" +
            "Continue?",
            "Keystroke Discovery",
        ...
        btn.Text = "Cancel Discovery";
        btn.BackColor = Color.FromArgb(150, 50, 50);

        var outputPath = Path.Combine(AppContext.BaseDirectory, "discovered_shortcuts.txt");

        _discovery = new KeystrokeDiscovery(msg =>
        {
            if (btn.IsDisposed) return;
            btn.Invoke(() => _mainForm.ShowStatusToast(msg, 3000));
        });
        ...
        System.Threading.Tasks.Task.Run(() =>
        {
            try
            {
                _discovery.RunDiscovery(outputPath);
            }
            finally
            {
                if (!btn.IsDisposed)
